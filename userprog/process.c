...
void start_process(void* filename_){
	...
	proc_stack->gs = 0;		//! can't output to screen in userspace
...
void page_dir_activate(struct task_struct* p_thread){
     ...
     ********************************************************/
	/* Page Dir of kernel */
	uint32_t page_phy_addr = 0x100000;
	/* Page Dir of user */
	if (p_thread->pgdir != NULL){
		page_phy_addr = addr_v2p((uint32_t)p_thread->pgdir);
	}
	/* Refresh CR3 to enable new page table */
